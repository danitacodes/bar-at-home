...
function getDrinkList () {
  const ingredient = document.querySelector('input').value.toLowerCase();
    ...
    fetch(`https://corsanywhere.herokuapp.com/https:www.thecocktaildb.com/api/json/v1/1/filter.php?i=${ingredient}`)
    .then (res => res.json())
    .then (data => {
        console.log(data)
        let html = "";
...
function getDrinkInstructions (e) {
  ...
  if(e.target.classList.contains('drink-btn')) {
    ...
    fetch(`https://corsanywhere.herokuapp.com/https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${drinkItem.dataset.id}`)
    .then (res => res.json())
